...
router.get('/login/:aname/:apwd',(req,res)=>{
   ...
   //需要对用户输入的密码执行加密函数
   pool.query('SELECT aid FROM res_admin WHERE aname=? AND apwd=PASSWORD(?)',[aname,apwd],(err,result)=>{
       if(err)throw err;
...
router.patch('/',(req,res)=>{
    ...
    pool.query('SELECT aid FROM res_admin WHERE aname=? AND apwd=PASSWORD(?)',[data.aname,data.oldPwd],(err,result)=>{
        if(err)throw err;
        if(result.length==0){ //查询到一行数据，登录成功
         ...
         //如果查询到了用户，在修改其密码
         pool.query('UPDATE res_admin SET apwd=PASSWORD(?) WHERE aname=?',[data.newPwd, data.aname],(err,result)=>{
             if(err)throw err;
             if(result.changedRows>0){ //密码修改完成
                res.send({code:200, msg: 'modified succ成功'})
             }else{ //新旧密码一样，未作修改
                res.send({code:400, msg: 'pwd not modified密码一样'})
             }
         })
    });
})
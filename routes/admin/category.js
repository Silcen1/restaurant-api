...
*/
router.get('/', (req, res) => {
    pool.query('SELECT * FROM res_category ORDER BY cid', (err, result) => {
        if (err) throw err;
...
router.delete('/:cid', (req, res) => {
    ...
    pool.query('UPDATE res_dish SET categoryId=NULL WHERE categoryId=?',
        req.params.cid, (err, result) => {
            if (err) throw err;
            //至此指定类别的菜品已经修改完毕
            pool.query('DELETE FROM res_category WHERE cid=?', req.params.cid, (err, result) => {
                if (err) throw err;
...
router.post('/', (req, res) => {
    ...
    pool.query('INSERT INTO res_category SET ?', data, (err, result) => {//注意此处SQL语句的简写
        if (err) throw err;
...
router.put('/', (req, res) => {
    var data = req.body;    //请求数据{code:xx, cname:'xx'}
    //TODO: 此处可以对数据进行验证
    pool.query('UPDATE res_category SET ? WHERE cid=?', [data, data.cid], (err, result) => {
        if (err) throw err;
        if (result.changedRows > 0) {// 实际更新了一行
            res.send({ code: 200, msg: '1 category modified' })
        } else if (result.affectedRows == 0) {
            res.send({ code: 400, msg: "category not exits" })
        } if (result.affectedRows == 1 && result.changedRows == 0) { //影响到1行，但修改了0行--新值与旧值完全一样
